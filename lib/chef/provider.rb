        ...
        @included_resource_dsl_module = Module.new do
          ...
          resource.class.properties.each do |name, property|
            class_eval(<<-EOM, __FILE__, __LINE__)
              def #{name}(*args, &block)
                # not making this a formal deprecation yet because I don't want to drive tim completely crazy, this
                # should be upgraded to a real deprecation in 13.1 or so
                Chef::Log.warn "Deprecated property access: please replace calling #{name} with new_resource.#{name}"
